...
专注「上传 Markdown → 同步到多个平台」，并支持直接发布。

- 🆕 新的同步页面：拖拽 / 选择 Markdown 文件或整个文件夹，支持多篇批量同步
- 🆕 解析 frontmatter（title / tags / category / summary / cover），第一个一级标题可作为标题
- 🆕 本地相对路径图片自动上传到各平台
- 🆕 直接发布：掘金、CSDN、知乎、博客园、语雀、WordPress、Typecho、MetaWeblog
- 🆕 直接发布失败时自动保留草稿并提示原因；不支持发布的平台保存为草稿
- 🗑️ 移除：网页文章提取、公众号/头条页面按钮、编辑器、同步弹窗、悬浮按钮、历史记录
- 🗑️ 移除：CLI、MCP Server、Claude Skill、数据统计、远程配置、版本检查、ZIP 下载、私有适配器子模块
